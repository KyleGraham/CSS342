...
void SList<Object>::insert( const Object &obj ) {
 ...
 static int counter = 1;
  ...
  SListNode<Object> *upper = NULL;
  SListNode<Object> *lower = NULL;
...
void SList<Object>::remove( const Object &obj ) {
  ...
  // item to delete
	if ( p->prev == NULL || p->next == NULL || p->item != obj )
		return;
	//removes the bottom level node
	p->prev->next = p->next;  // found->prev != null
	if ( p->next != NULL )
		p->next->prev = p->prev;
	//check if the item had up values
	while(p->up != NULL)
	{
		//deletes any upper level nodes
		p = p->up;
		p->prev->next = p->next;  // found->prev != null
		if ( p->next != NULL )
			p->next->prev = p->prev;
