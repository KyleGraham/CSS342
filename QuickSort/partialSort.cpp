...
template <class Comparable>
void partialSort( vector<Comparable> &a, int first, int last, int topK) {
  int pivotIndex;
  if(first > topK)
	  return;
  ...
  if ( first < last ) {
    ...
    partialSort( a, first, pivotIndex - 1, topK );
    partialSort( a, pivotIndex + 1, last, topK );
  }
}
template <class Comparable>
void partialSort( vector<Comparable> &a , int topK) {
  partialSort( a, 0, a.size( ) - 1, topK);
}